export const myProjects = [
  {
    id: 1,
    title: "TicTacToe",
    description:
      "Developed an interactive Tic Tac Toe game using React with smooth UI, game state management, and winner detection logic.",
    subDescription: [
      "Implemented game logic to handle player turns, win detection, and draw conditions using React state management.",
      "Designed a responsive and intuitive user interface with reusable React components for the game board and squares.",
      "Added interactive UI updates to reflect real-time gameplay and highlight winning combinations.",
      "Ensured clean code structure and component reusability for maintainability and scalability.",
    ],
    href: "",
    logo: "",
    image: "/assets/projects/tic tac toe.jpeg",
    tags: [
      
      {
        id: 2,
        name: "React.js",
        path: "/assets/logos/react.svg",
      },
      {
        id: 3,
        name: "JavaScript",
        path: "/assets/logos/javascript.svg",
      },
      {
        id: 4,
        name: "CSS",
        path: "/assets/logos/css3.svg",
      },
      {
        id: 5,
        name: "VsCode",
        path: "/assets/logos/visualstudiocode.svg",
      },
    ],
  },
  {
    id: 2,
    title: "HRMS Portal",
    description:
      "Built a scalable HRMS portal with role-based access, employee management, attendance tracking, and payroll features.",
    subDescription: [
      "Developed responsive user interfaces for employee management, attendance tracking, and leave management modules.",
      "Implemented role-based access control to differentiate functionalities for admins, HR managers, and employees.",
      "Integrated dynamic forms and dashboards to display employee data, payroll details, and organizational information.",
      "Ensured smooth data flow between frontend and backend through secure API integration.",
    ],
    href: "",
    logo: "",
    image: "/assets/projects/hrms.jpeg",
    tags: [
      {
        id: 1,
        name: "JavaScript",
        path: "/assets/logos/javascript.svg",
      },
      {
        id: 2,
        name: "React.js",
        path: "/assets/logos/react.svg",
      },
      {
        id: 3,
        name: "API",
        path: "/assets/logos/api.svg",
      },
      {
        id: 4,
        name: "CSS",
        path: "/assets/logos/css3.svg",
      },
    ],
  },
  {
    id: 3,
    title: "Weather App",
    description:
      "Created a modern weather application providing accurate, real-time weather updates with an intuitive and responsive user interface.",
    subDescription: [
      "Integrated third-party weather APIs to fetch real-time temperature, humidity, and forecast data based on user location.",
      "Designed a responsive and user-friendly interface optimized for both desktop and mobile devices.",
      "Implemented search functionality to allow users to view weather conditions for different cities.",
      "Handled API loading states and error scenarios to ensure a smooth user experience.",
    ],
    href: "",
    logo: "",
    image: "/assets/projects/weather app.jpeg",
    tags: [
      {
        id: 1,
        name: "API",
        path: "/assets/logos/api.svg",
      },
      {
        id: 2,
        name: "Vs Code",
        path: "/assets/logos/visualstudiocode.svg",
      },
      {
        id: 3,
        name: "React.js",
        path: "/assets/logos/react.svg",
      },
      {
        id: 4,
        name: "TailwindCSS",
        path: "/assets/logos/tailwindcss.svg",
      },
    ],
  },
  {
    id: 4,
    title: "Meme Generator",
    description:
      "Created a user-friendly meme generator app enabling users to generate and share memes with customizable captions.",
    subDescription: [
      "Implemented image selection and preview functionality to provide instant visual feedback during meme creation.",
      "Managed application state efficiently to ensure smooth updates and seamless user interactions.",
      "Designed a responsive layout to support meme generation across different screen sizes and devices.",
      "Focused on clean UI, usability, and performance optimization for an engaging user experience.",
    ],
    href: "",
    logo: "",
    image: "/assets/projects/meme creator.jpeg",
    tags: [
      {
        id: 1,
        name: "API",
        path: "/assets/logos/api.svg",
      },
      {
        id: 2,
        name: "Vs Code",
        path: "/assets/logos/visualstudiocode.svg",
      },
      {
        id: 3,
        name: "React.js",
        path: "/assets/logos/react.svg",
      },
      {
        id: 4,
        name: "TailwindCSS",
        path: "/assets/logos/tailwindcss.svg",
      },
    ],
  },
  {
    id: 5,
    title: "To Do App",
    description:
      "Developed a to-do application to manage daily tasks with add, update, and delete functionality in a clean interface.",
    subDescription: [
      "Managed application state efficiently to ensure real-time updates and smooth task interactions.",
      "Added task status indicators to help users track completed and pending tasks easily.",
      "Designed a responsive layout to support to-do list management across different screen sizes and devices.",
      "Focused on clean UI, usability, and performance optimization for an engaging user experience.",
    ],
    href: "",
    logo: "",
    image: "/assets/projects/to do app.jpeg",
    tags: [
      {
        id: 1,
        name: "JSX",
        path: "/assets/logos/javascript.svg",
      },
      {
        id: 2,
        name: "CSS",
        path: "/assets/logos/css3.svg",
      },
      {
        id: 3,
        name: "React.js",
        path: "/assets/logos/react.svg",
      },
    ],
  },
];

export const mySocials = [
  {
    name: "WhatsApp",
    href: "https://wa.me/7665555027",
    icon: "/assets/socials/whatsApp.svg",
  },
  {
    name: "Linkedin",
    href: "https://www.linkedin.com/in/yashraj-singh-chauhan-/",
    icon: "/assets/socials/linkedIn.svg",
  },

];

export const experiences = [
  {
    title: "Generative AI Developer Intern",
    job: "AI Chat Bots",
    date: "Jan 2025 - Jul 2025",
    contents: [
      "Designed and developed Generative AI–powered chatbots with a strong focus on application security, ensuring compliance with organizational cybersecurity standards and secure data handling practices.",
      "Implemented intelligent conversational flows using NLP and large language models (LLMs), enabling accurate intent recognition, contextual responses, and real-time user interaction.",
      "Built interactive chatbot interfaces with intuitive UI/UX principles, integrating conversational agents seamlessly into web applications to enhance user engagement and accessibility.",
      "Developed AI-driven automation features to streamline industrial and enterprise workflows, enabling chatbots to interact with backend systems and retrieve or process operational data dynamically.",
      "Optimized chatbot performance through prompt engineering, response tuning, and model evaluation, improving accuracy, relevance, and overall user satisfaction.",
    ],
  },
  {
    title: "Full - Stack Developer Intern",
    job: "Web-App Development",
    date: "Jan 2026 - Present",
    contents: [
      "Designed and developed responsive, high-performance user interfaces for data-intensive applications, ensuring smooth visualization of large-scale, real-time data.",
      "Built interactive dashboards to display telemetry, diagnostics, and live status information using modern frontend frameworks and component-based architecture.",
      "Implemented real-time UI updates by integrating frontend components with backend APIs and WebSocket-based data streams.",
      "Focused on UX consistency and accessibility, delivering intuitive layouts and smooth user interactions across devices and screen sizes.",
      "Integrated secure API communication on the frontend, handling authentication states, error handling, and protected data rendering.",
    ],
  },
  
];
export const reviews = [
  {
    name: "Vijay",
    username: "@vijay",
    body: "I've never seen anything like this before. It's amazing. I love it.",
    img: "https://robohash.org/jack",
  },
  {
    name: "Amit",
    username: "@amit",
    body: "I don't know what to say. I'm speechless. This is amazing.",
    img: "https://robohash.org/jill",
  },
  {
    name: "Priya",
    username: "@priya",
    body: "I'm at a loss for words. This is amazing. I love it.",
    img: "https://robohash.org/john",
  },
  {
    name: "Sandeep",
    username: "@sandeep",
    body: "This is hands down the best thing I've experienced. Highly recommend!",
    img: "https://robohash.org/alice",
  },
  {
    name: "Kavita",
    username: "@kavita",
    body: "Incredible work! The attention to detail is phenomenal.",
    img: "https://robohash.org/bob",
  },
  {
    name: "Vikas",
    username: "@vikas",
    body: "This exceeded all my expectations. Absolutely stunning!",
    img: "https://robohash.org/charlie",
  },
  {
    name: "Dave",
    username: "@dave",
    body: "Simply breathtaking. The best decision I've made in a while.",
    img: "https://robohash.org/dave",
  },
  {
    name: "Rohit",
    username: "@rohit",
    body: "So glad I found this. It has changed the game for me.",
    img: "https://robohash.org/eve",
  },
];




// Create the cursor element
const cursor = document.createElement('div');
cursor.classList.add('cursor');
document.body.appendChild(cursor);

// Make it follow the mouse
document.addEventListener('mousemove', (e) => {
    cursor.style.left = e.clientX + 'px';
    cursor.style.top = e.clientY + 'px';
});
document.querySelectorAll('a, button').forEach(el => {
    el.addEventListener('mouseenter', () => cursor.classList.add('cursor-hover'));
    el.addEventListener('mouseleave', () => cursor.classList.remove('cursor-hover'));
});






